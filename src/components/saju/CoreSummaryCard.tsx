...
export default function CoreSummaryCard({ pillars }: CoreSummaryCardProps) {
  ...
  // 십신 5분류 카운트
  const catLabels = ['비겁', '식상', '재성', '관성', '인성'] as const;
  const catMap: Record<string, number> = { '비겁': 0, '식상': 0, '재성': 0, '관성': 0, '인성': 0 };
  const categoryOf: Record<string, string> = {
    '비견': '비겁', '겁재': '비겁', '식신': '식상', '상관': '식상',
    '편재': '재성', '정재': '재성', '편관': '관성', '정관': '관성',
    '편인': '인성', '정인': '인성',
  };
  for (const [god, count] of Object.entries(tenGodSummary.distribution)) {
    catMap[categoryOf[god]] += count;
  }
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-950/80 to-purple-950/80 rounded-2xl p-4 border border-indigo-800/50">
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xl font-bold text-white">{dm.stem}({dm.hanja})</span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-800/50 text-indigo-300">
              {dm.element} · {dm.nickname}
            </span>
          </div>
          <p className="text-sm text-gray-300">{dm.shortDesc}</p>
        </div>
      </div>
      ...
      {/* 십신 분포 바 차트 */}
      <div className="bg-black/20 rounded-xl p-3 mb-3">
        <p className="text-xs font-bold text-gray-400 mb-2">십신 에너지 분포</p>
        <div className="space-y-1.5">
          {catLabels.map(cat => {
            const count = catMap[cat];
            const pct = (count / maxCat) * 100;
            const colors: Record<string, string> = {
              '비겁': 'bg-blue-500', '식상': 'bg-green-500',
              '재성': 'bg-yellow-500', '관성': 'bg-red-500', '인성': 'bg-purple-500',
            };
            return (
              <div key={cat} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-4 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${colors[cat]} rounded-full transition-all duration-500`}
      ...
      {/* 성격 상세 */}
      <div className="bg-black/20 rounded-xl p-3 mb-3">
        <p className="text-sm text-gray-300 leading-relaxed">{dm.personality}</p>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-bold text-gray-400">종합 분석</span>
          <span className={`text-gray-500 text-xs transition-transform ${expanded ? 'rotate-180' : ''}`}>▼</span>
        </div>
        <p className="text-sm text-gray-300">{tenGodSummary.personalitySummary}</p>
